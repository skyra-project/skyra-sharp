...
namespace Skyra.Worker.Commands
{
	...
	public sealed class ReplyCommand : StructureBase
	{
		...
		public async Task RunAsync([NotNull] Message message, [Argument(Minimum = -5)] int number)
		{
			await message.SendAsync($"A number! {number.ToString()}");
		}

		public async Task RunAsync([NotNull] Message message, DateTime time,
		...
		public async Task RunAsync([NotNull] Message message, string content, [NotNull] int[] integers)
		{
			await message.SendAsync(
